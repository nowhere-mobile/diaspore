...
#
# su:s0 (like nowhere_provision) so `am` can start an activity. Idempotent with the BootReceiver:
...
i=0
while [ "$i" -lt 4 ]; do
  am start -n com.nowhere.chooser/.ChooserActivity >/dev/null 2>&1 && break
  i=$((i + 1)); sleep 1
done
