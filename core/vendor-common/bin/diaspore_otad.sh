#!/system/bin/sh
# Diaspore on-device OTA updater (su:s0) -- DIA-20260618-03, "self-service OTA". On-demand worker (an app
# can't spawn su:s0): the caller writes $REQ then `setprop ctl.start diaspore_otad`; init runs this once and
...
# The OS is public + content-addressed, so this carries NO profile creds (unlike diaspore_roamd).
STATE=/data/diaspore/state
REQ="$STATE/ota.req"
RES="$STATE/ota.res"
LOG=/data/diaspore/ota.log                 # status only
...
OSREF=os-fp3 ; OSPASS=diaspore-os          # well-known ref for the OS payload (the pass just keys the public seal)
...
export DIASPORE_CHUNK_CACHE=/data/diaspore/otacache   # delta download: only fetch changed chunks (DIA-20260618-02)
