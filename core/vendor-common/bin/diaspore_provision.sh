#!/system/bin/sh
# Diaspore first-boot provisioning: make the chooser the DEVICE OWNER so it can drive the kiosk Lock
# Task on the gate -- with NO manual adb. Runs once per fresh /data (a marker, cleared by a factory
# reset). su:s0 (not the confined diaspore domain) because dpm/settings need broad framework access.
LOG=/data/diaspore/provision.log
MARK=/data/diaspore/.provisioned
ADMIN=com.diaspore.chooser/.AdminReceiver
...
i=0
while [ "$i" -lt 10 ]; do
  pm path com.diaspore.chooser >/dev/null 2>&1 && break
  i=$((i + 1)); sleep 2
done
...
# service can write the reply. From this su:s0 init service those fds point at our LOG (diaspore_data_file) or,
...
owner_ok() {
  for f in /data/system/device_owner_2.xml /data/system/device_owner.xml; do
    [ -f "$f" ] && grep -q "com.diaspore.chooser" "$f" 2>/dev/null && return 0
  done
  return 1
}
...
am start -n com.diaspore.chooser/.ChooserActivity </dev/null >/dev/null 2>&1
...
# Disable the LineageOS SetupWizard on user 0. On a provisioned Diaspore device the blind-login gate IS the
