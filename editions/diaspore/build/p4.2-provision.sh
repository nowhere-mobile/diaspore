...
"$AG/nowhere_agent" push-set s3 "$PROFILE" "$PASS" "$W/seed"

echo "=== verify round-trip (working set) ==="
"$AG/nowhere_agent" restore-set s3 "$PROFILE" "$PASS" /tmp/provverify 1
...
mkdir -p "$VD/etc"
cat > "$VD/etc/nowhere.conf" <<EOF
S3_ENDPOINT=$S3_ENDPOINT
S3_REGION=$S3_REGION
S3_BUCKET=$S3_BUCKET
S3_ACCESS_KEY=$S3_ACCESS_KEY
S3_SECRET_KEY=$S3_SECRET_KEY
NOWHERE_PROFILE=$PROFILE
NOWHERE_PASS=$PASS
EOF
chmod 600 "$VD/etc/nowhere.conf"
ls -la "$VD/etc/nowhere.conf"
echo "RESULT=$SEED"
